...
VOID Callback(PDEVICE_OBJECT DeviceObject, PWORKITEM_CONTEXT context);
...
--*/
{
    ...
    if (InputDataStart->ButtonFlags & MOUSE_LEFT_BUTTON_DOWN)
    {
        PWORKITEM_CONTEXT context = (PWORKITEM_CONTEXT)ExAllocatePool(NonPagedPool, sizeof(WORKITEM_CONTEXT));
        if (context != NULL)
        {
            context->workItem = IoAllocateWorkItem(DeviceObject);
            context->data = InputDataStart;
            if (context->workItem != NULL)
            {
                IoQueueWorkItem(context->workItem, (PIO_WORKITEM_ROUTINE)Callback, DelayedWorkQueue, context);
            }
        }
    }
...
{
    PAGED_CODE();
    DebugPrint(("Callback run successfully"));
    ...
    if (context != NULL)
    {
        ...
        RtlInitUnicodeString(&uniName, L"\\SystemRoot\\example.txt");
        ...
        LONG x = context->data->LastX;
        LONG y = context->data->LastY;
        WCHAR buffer[30];
        size_t s;
        if (NT_SUCCESS(ntstatus)) {
            ntstatus = RtlStringCbPrintfA(buffer, sizeof(buffer), "X: %ld; Y: %ld\n", x, y);
            if (NT_SUCCESS(ntstatus)) {
                ntstatus = RtlStringCbLengthA(buffer, sizeof(buffer), &s);
                if (NT_SUCCESS(ntstatus)) {
                    ntstatus = ZwWriteFile(handle, NULL, NULL, NULL, &ioStatusBlock,
                        buffer, s, &ByteOffset, NULL);
                }
            }
            ZwClose(handle);
        }
